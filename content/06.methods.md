...
We preprocessed raw Illumina Infinium Human Methylation Bead Arrays (450K and EPIC(850K)) intensities using the array preprocessing methods implemented in the [minfi Bioconductor package](https://bioconductor.org/packages/release/bioc/html/minfi.html). 
We utilized either `preprocessFunnorm` when an array dataset has control samples (i.e., normal and tumor samples) or multiple OpenPedcan cancer groups and `preprocessQuantile` when an array dataset has only tumor samples from a single OpenPedcan cancer group to estimate usable methylation measurements (`beta-values` and `m-values`) and copy number (`cn-values`). 
...
In addition, each CpG probe is annotated with a gene feature to identify the genomic regions likely involved in regulating gene expression. 

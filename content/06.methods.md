...
#### Gene Expression
The [`tumor-normal-differential-expression`](https://github.com/PediatricOpenTargets/OpenPedCan-analysis/tree/dev/analyses/tumor-normal-differential-expression) module perfoms differential expression analyses for all sets of Disease and Dataset across all genes found in the `gene-expression-rsem-tpm-collapsed.rds` table. 
...
This analysis was performed on the Children's Hospital of Philadelphia HPC and was configured to use 96G of RAM per CPU, with one task (one iteration of expression analysis for each set of tissue and cancer group) per CPU (total 102*52=5304 CPUs) using the R/DESeq2 library. 
...
The same analysis can also be performed on CAVATICA.
